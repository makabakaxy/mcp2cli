...
> 源自 [5.0-cli-generation.md](5.0-cli-generation.md) 的 5.3 和 5.4 节。
...
```yaml
server: <server-name>
version: "<来自 tools JSON 的 server 版本号，若无则为 null>"
...
| 字段 | 位置 | 必需 | 说明 |
|------|------|------|------|
| `version` | 顶层 | 是 | 来自 tools/*.json 中的 server 版本号，用于检测 server 升级。若 server 未提供则为 `null` |
...
3. **叶子节点**：必有 `_tool` 和 `_description`，可选 `_examples`
