...
└──────────────────┬──────────────────────┘
                   ...
                   ▼
┌──────────────────────────────────────────┐
│  ~/.agents/mcp2cli/tools/                │
│    ├── mcp-atlassian.json               │
│    ├── playwright.json                  │
│    └── ...                              │
│  (每个 server 独立文件, 含完整 tool      │
│   列表 + input_schema + version)        │
└──────────────────┬───────────────────────┘
...
| 层级 | 类型 | 命令形式 |
|------|------|----------|
| 第一层（管理） | 管理命令 | `mcp2cli list / scan / generate / daemon` |
| 第二层（直通） | 底层直通命令 | `mcp2cli tools <server> [tool]`<br>`mcp2cli call <server> <tool> [--args]` |
| 第三层（层级） | 层级式命令（AI 生成，渐进式披露） | `mcp2cli <server> <group> <sub> <action>` |
...
# 生成 SKILL.md（基于层级命令树）
mcp2cli generate skill mcp-atlassian [-o ~/.agent/skills/mcp-atlassian/]
...
详细设计见 [5-ai-generate-cli.md](5-ai-generate-cli.md)（Prompt 模板、Skill/Example 文件内容、可配置 LLM 后端、--merge 模式、校验流程）。
...
`mcp2cli generate skill` 读取 CLI 映射文件和 tool schema，通过 AI 生成 agent 可用的 Skill 文件集合：
...
```
~/.agent/skills/<server>/
├── SKILL.md                        # 主文件（≤400 tokens），agent 每次对话加载
...
- **精简**：≤400 tokens，只含命令总览、快捷方式、2-3 个示例
...
详细设计见 [6-skill-generation.md](6-skill-generation.md)（Prompt 模板、skill_gen_skill.md 和 skill_gen_example.md 完整内容、reference 文件格式规范、校验流程）。
...
| mcp2cli + Skill | SKILL.md ≈ 400 tokens + 按需 reference ≈ 200 tokens/次<br>一般一次对话调用 2-3 个 tool ≈ 1000 tokens，**节省 80%** |
...
│       └── templates/
│           ├── cli_gen_skill.md      # 生成规则和约束（LLM 读取）
│           ├── cli_gen_example.md    # 示例 YAML 输出（LLM 读取）
...
$ mcp2cli list
  ...
  mcp-atlassian    1.2.3      claude:global    not scanned
...
$ mcp2cli generate skill mcp-atlassian
  Skill files generated to ~/.agent/skills/mcp-atlassian/
...
- `mcp2cli install`：一键安装 MCP server，AI 搜索配置信息，自动写入 Claude/Cursor/Codex 三个配置文件，并执行 scan + generate 生成层级式 CLI 命令。详细设计见 [11-install.md](11-install.md)。
