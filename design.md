...
# 扫描 server 的 tool 列表，写入 registry/<server>.json，返回文件路径
...
解析过程:
  ...
  5. 剩余参数 "--project INFRA" → 转为 tool 参数
     ...
     → 查 registry/mcp-atlassian.json 中 jira_create_issue 的 input_schema 校验参数合法性
  6. 转发到 daemon:
...
解析:
  ...
  4. 在 registry/mcp-atlassian.json 中查找 tool 是否存在
  5. 存在则直接调用，不存在则报错
```
...
mcp2cli generate cli mcp-atlassian
            │
            ▼
  ┌─ 1. 读取 registry/mcp-atlassian.json ────────┐
...
| mcp2cli + Skill | SKILL.md ≈ 400 tokens + 按需 `--help` ≈ 200 tokens/次<br>一般一次对话调用 2-3 个 tool ≈ 1000 tokens，**节省 80%** |
...
├── registry/                         # tool schema 物化视图（每 server 独立文件）
│   ├── mcp-atlassian.json
│   └── playwright.json
...
│   │   ├── registry.py               # registry/ 文件夹管理（读写 per-server JSON）
...
$ mcp2cli scan mcp-atlassian
  ...
  Found 12 tools. Written to ~/.agents/mcp2cli/registry/mcp-atlassian.json
