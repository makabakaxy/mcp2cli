  ...
  jira:
    ...
    issue:
      ...
      create:
        ...
        _param_aliases:             # 可选：参数名别名
          issue-type: issue_type    # CLI 风格(kebab) → MCP 原始名(snake)
      get:
...
_param_aliases 将 CLI 风格的 kebab-case 映射到 MCP 的 snake_case
...
解析过程:
  ...
  5. 剩余参数 "--project INFRA" → 转为 tool 参数
     → 查 _param_aliases: project → project (无别名, 原样)
     → 查 registry.json 中 jira_create_issue 的 input_schema
     → 校验参数合法性
  6. 转发到 daemon:
...
参数命名风格	CLI 用 kebab-case，映射到 MCP 的 snake_case	符合 CLI 惯例，_param_aliases 处理映射
